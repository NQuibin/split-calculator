...
export function orderExpensePeople(
  expense: Pick<Doc<"expenses">, "roundingOrder">,
  people: Person[],
) {
  const order = expense.roundingOrder ?? [];
  const priority = new Map<string, number>(order.map((memberId, index) => [memberId, index]));
  return [...people].sort(
    (a, b) => (priority.get(a.id) ?? order.length) - (priority.get(b.id) ?? order.length),
  );
}
...
export async function resolveExpenseMembers(ctx: QueryCtx | MutationCtx, expense: Doc<"expenses">) {
  ...
  const people = await Promise.all(
    seats.map(async (seat) => {
      ...
      return {
        id: seat._id as string,
        name: user?.name?.trim() || user?.email?.trim() || seat.name,
      };
    }),
  );
  return {
    ...expense,
    people: orderExpensePeople(expense, people),
  };
}
