...
const legacySchema = defineSchema({
  ...
  expenses: defineTable({
    ...schema.tables.expenses.validator.fields,
...
test("backfill uses creator identity, preserves explicit payers and expense data, and is repeatable", async () => {
  ...
  const { missing, explicit, creatorSeat, before } = await t.run(async (ctx) => {
    ...
    const tab = await ctx.db.insert("tabs", {
      slug: "trip",
      name: "Trip",
      ownerUserId: creator,
      updatedAt: 1,
    });
    const otherSeat = await ctx.db.insert("tabMembers", {
      tabId: tab,
      name: "Other",
      inviteToken: "other",
    });
    const creatorSeat = await ctx.db.insert("tabMembers", {
      tabId: tab,
      name: "Creator",
      userId: creator,
      inviteToken: "creator",
    ...
    const explicit = await ctx.db.insert("expenses", {
      ...data,
      slug: "explicit",
      payerId: otherSeat,
    });
    return {
      ...
      explicit,
      creatorSeat,
      before: [await ctx.db.get(missing), await ctx.db.get(explicit)],
    };
  });
  await t.run((ctx) =>
    runToCompletion(ctx, components.migrations, internal.payerMigration.backfillCreatorPayer),
  );
  ...
  expect(
    await t.query(internal.payerMigration.audit, {
      paginationOpts: { cursor: null, numItems: 100 },
    }),
  ).toMatchObject({ total: 2, missing: 0, invalid: 0, unmappedCreators: 0, isDone: true });
});
