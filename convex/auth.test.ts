...
test("email sessions require a password and a fresh OTP, including for verified accounts", async () => {
  ...
  const legacyUser = await t.run((ctx) => ctx.db.insert("users", { email: "test@example.com", emailVerificationTime: Date.now() }));
  ...
  await expect(signIn({ ...credentials, password: "wrong", code, flow: "verify" })).rejects.toThrow();
  ...
  expect(sessions[0].expirationTime - sessions[0]._creationTime).toBeCloseTo(30 * 24 * 60 * 60 * 1000, -3);
  ...
  expect((await signIn({ email: "new@example.com", password: "another-password", flow: "signUp" })).tokens).toBeTruthy();
