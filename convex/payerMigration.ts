...
export const backfillCreatorPayer = migrations.define({
  ...
  migrateOne: async (ctx, expense) => {
    ...
    const seats = await ctx.db.query("tabMembers")
      .withIndex("by_tab", q => q.eq("tabId", expense.tabId!)).take(501);
    ...
    if (creators.length !== 1) throw new Error(`Expense ${expense._id} needs exactly one creator seat`);
...
export const audit = internalQuery({
  ...
  returns: v.object({ total: v.number(), missing: v.number(), invalid: v.number(), unmappedCreators: v.number(), continueCursor: v.string(), isDone: v.boolean() }),
  handler: async (ctx, { paginationOpts }) => {
    const page = await ctx.db.query("expenses").paginate(paginationOpts);
    let missing = 0, invalid = 0, unmappedCreators = 0;
    for (const expense of page.page) {
      if (expense.payerId) {
        ...
        missing++;
        const seats = expense.tabId ? await ctx.db.query("tabMembers")
          .withIndex("by_tab", q => q.eq("tabId", expense.tabId!)).take(501) : [];
        if (seats.length > 500 || seats.filter(seat => seat.userId === expense.userId).length !== 1) unmappedCreators++;
      }
    }
    return { total: page.page.length, missing, invalid, unmappedCreators, continueCursor: page.continueCursor, isDone: page.isDone };
  },
});
