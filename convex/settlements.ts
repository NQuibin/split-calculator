...
function paymentCents(amount: number) {
  ...
  if (!Number.isFinite(amount) || rounded <= 0 || !Number.isSafeInteger(rounded) || Math.abs(value - rounded) > 1e-7) {
...
async function limited<T>(value: Promise<T[]>, name: string) {
  ...
  if (rows.length > READ_LIMIT) throw new Error(`Settlement balances support up to ${READ_LIMIT} ${name} per tab. This tab needs a larger ledger before it can be settled.`);
  return rows;
}

async function findTab(ctx: ReadCtx, slug: string) {
  return await ctx.db.query("tabs").withIndex("by_slug", q => q.eq("slug", slug)).unique();
}

function blank(roster: Doc<"tabMembers">[]): CurrencyTotals {
  return new Map(roster.map(seat => [seat._id, { paidCents: 0, shareCents: 0, transferredCents: 0 }]));
}
...
async function readBalances(ctx: ReadCtx, tab: Doc<"tabs">, asOfDate: string) {
  const [roster, expenses, payments] = await Promise.all([
    limited(ctx.db.query("tabMembers").withIndex("by_tab", q => q.eq("tabId", tab._id)).take(READ_LIMIT + 1), "members"),
    limited(ctx.db.query("expenses").withIndex("by_tab", q => q.eq("tabId", tab._id)).take(READ_LIMIT + 1), "expenses"),
    limited(ctx.db.query("settlements").withIndex("by_tabId", q => q.eq("tabId", tab._id)).take(READ_LIMIT + 1), "payments"),
  ]);
  const people = await Promise.all(roster.map(async seat => ({ id: seat._id, name: await resolveSeatName(ctx, seat) })));
  ...
  for (const expense of expenses) {
    ...
    if (expense.items.some(item => item.splitWith.length === 0
      || new Set(item.splitWith).size !== item.splitWith.length
      || item.splitWith.some(id => !memberIds.has(id)))) {
      throw new Error(`Expense "${expense.name || expense.slug}" needs a valid split before settling`);
    }
    const split = computeSplit(orderExpensePeople(expense, people), expense.items, expense.globalAdjustments);
    ...
    const shares = rate ? convertShares(computeShares(split), split.grandTotal, rate.rate) : computeShares(split);
    const shareAmounts = shares.map(share => checkedCents(share.fairShare));
    if (shareAmounts.reduce(addCents, 0) !== totalCents) throw new Error(`Expense "${expense.name || expense.slug}" has inconsistent shares`);
...
const settlementResult = v.union(v.null(), v.object({
  currencies: v.array(v.object({
    currency: v.string(),
    members: v.array(v.object({ memberId: v.id("tabMembers"), name: v.string(), paid: v.number(), share: v.number(), balance: v.number() })),
    suggestions: v.array(v.object({ fromMemberId: v.id("tabMembers"), toMemberId: v.id("tabMembers"), amount: v.number() })),
  })),
  missingPayers: v.array(v.object({ slug: v.string(), name: v.string() })),
  history: v.array(v.object({
    id: v.id("settlements"), fromMemberId: v.id("tabMembers"), toMemberId: v.id("tabMembers"),
    amount: v.number(), currency: v.string(), date: v.string(), note: v.optional(v.string()), reversed: v.boolean(),
  })),
...
export const get = query({
  ...
  handler: async (ctx, { slug, asOfDate }) => {
    ...
    const { roster, people, byCurrency, missingPayers, payments } = await readBalances(ctx, tab, asOfDate);
    return {
      currencies: [...byCurrency].sort(([a], [b]) => a.localeCompare(b)).map(([currency, totals]) => {
        const members = people.map(person => {
          const total = totals.get(person.id)!;
          return { memberId: person.id, name: person.name, paid: total.paidCents / 100, share: total.shareCents / 100, balance: net(total) / 100 };
        });
        return { currency, members, suggestions: suggestSettlements(members).map(suggestion => ({
          ...suggestion,
      ...
      missingPayers,
      history: payments.filter(payment => payment.date <= asOfDate)
        .sort((a, b) => b.date.localeCompare(a.date) || b._creationTime - a._creationTime)
        .map(payment => ({
          id: payment._id, fromMemberId: payment.fromMemberId, toMemberId: payment.toMemberId,
          amount: payment.amountCents / 100, currency: payment.currency, date: payment.date,
          note: payment.note, reversed: payment.reversedAt !== undefined,
        })),
...
export const record = mutation({
  args: {
    slug: v.string(), fromMemberId: v.id("tabMembers"), toMemberId: v.id("tabMembers"),
    amount: v.number(), currency: v.string(), date: v.string(), note: v.optional(v.string()),
    requestId: v.string(), asOfDate: v.string(),
  },
  returns: v.null(),
  handler: async (ctx, args) => {
    ...
    if (args.fromMemberId === args.toMemberId) throw new Error("Settlement members must be distinct");

    const prior = await ctx.db.query("settlements")
      .withIndex("by_tabId_and_requestId", q => q.eq("tabId", tab._id).eq("requestId", requestId)).unique();
    if (prior) {
      if (prior.fromMemberId === args.fromMemberId && prior.toMemberId === args.toMemberId
        && prior.amountCents === amountCents && prior.currency === args.currency
        && prior.date === args.date && prior.note === note) return null;
    ...
    if (!ids.has(args.fromMemberId) || !ids.has(args.toMemberId)) throw new Error("Settlement members must belong to this tab");
    ...
    if (fromBalance >= 0 || toBalance <= 0 || amountCents > -fromBalance || amountCents > toBalance) {
      throw new Error("Settlement amount exceeds the current amount owed");
    }
    await ctx.db.insert("settlements", {
      tabId: tab._id, fromMemberId: args.fromMemberId, toMemberId: args.toMemberId,
      amountCents, currency: args.currency, date: args.date, note, recordedBy: userId, requestId,
    });
    return null;
  },
});
