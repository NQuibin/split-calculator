...
export function CurrencyFilter({ value, onChange, codes, label }: {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger render={<Button variant="field" aria-label={label} className="group min-h-11 shrink-0 gap-3 rounded-lg px-3 text-xs" />}>
      ...
      <PopoverContent align="end" className="w-44 rounded-lg p-2">
        <ul className="max-h-56 space-y-0.5 overflow-y-auto">
          {["all", ...codes].map(code => (
            <li key={code}>
              <MenuOption selected={value === code} onClick={() => { onChange(code); setOpen(false); }}>
