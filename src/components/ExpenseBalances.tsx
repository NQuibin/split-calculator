...
export function ExpenseBalances({ people, split, payerId, currency, projected = false, unallocated = false, headingLevel = "h2" }: { people: Person[]; split: SplitResult; payerId?: string; currency: string; projected?: boolean; unallocated?: boolean; headingLevel?: "h2" | "h3" }) {
  ...
  return <section className="mt-6 border-t border-rule pt-5" aria-label="Expense balances">
    ...
    <div className="mt-3 overflow-x-auto rounded-md border border-edge bg-field">
      <table className="w-full min-w-[28rem] text-sm"><thead className="bg-band text-left text-xs text-ink-soft"><tr><th className="px-3 py-2 font-medium">Person</th><th className="px-3 py-2 text-right font-medium">Paid</th><th className="px-3 py-2 text-right font-medium">Share</th><th className="px-3 py-2 text-right font-medium">Balance</th></tr></thead><tbody className="divide-y divide-rule">{rows.map((row) => <tr key={row.memberId}><th scope="row" className="px-3 py-3 text-left font-medium"><span className="flex items-center gap-2"><MemberAvatar id={row.memberId} name={row.name} /><span className="break-words">{row.name}</span></span></th><td className="font-numeric px-3 py-3 text-right">{formatCurrency(row.paid, currency)}</td><td className="font-numeric px-3 py-3 text-right">{formatCurrency(row.share, currency)}</td><td className={`font-numeric px-3 py-3 text-right ${row.balance > 0 ? "text-ledger-green" : row.balance < 0 ? "text-margin-red-ink" : "text-ink"}`}><span className="inline-flex items-center justify-end gap-1 whitespace-nowrap">{row.balance > 0 ? `Gets ${formatCurrency(row.balance, currency)}` : row.balance < 0 ? `Owes ${formatCurrency(Math.abs(row.balance), currency)}` : projected ? "Not due" : "Settled"}{row.balance > 0 ? <MoveUp aria-hidden="true" className="h-5 w-5 shrink-0" strokeWidth={2.5} /> : row.balance < 0 ? <MoveDown aria-hidden="true" className="h-5 w-5 shrink-0" strokeWidth={2.5} /> : null}</span></td></tr>)}</tbody></table>
    </div>
