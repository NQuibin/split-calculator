...
function TabLink({ slug, name }: { slug: string; name: string }) {
  return (
    ...
    >
      ...
      <ChevronRight
        aria-hidden="true"
        className="h-3.5 w-3.5 shrink-0 self-center chevron-x"
        strokeWidth={2.25}
      />
    </Link>
  );
...
export function ExpenseTabField({
  tabs,
  value,
  name,
  loading,
  ...
  saved,
  onChange,
}: ExpenseTabFieldProps) {
  const [open, setOpen] = useState(false);
  const [creating, setCreating] = useState(false);
  const label = name ?? tabs.find((tab) => tab.slug === value)?.name;

  return (
    <section className="mt-2 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 text-sm">
      <p id="expense-tab-label" className="shrink-0 text-sm text-ink-soft">
        In tab ·
      </p>
      {saved ? (
        <p className="text-sm text-ink">
          {value && label ? <TabLink slug={value} name={label} /> : "Personal expense"}
        </p>
      ) : locked ? (
        <>
          {/* No aria-labelledby here: a <p> is generic, so the association was
          dropped anyway, and the "In tab ·" label sits immediately before this
          in reading order - same as the saved branch above. */}
          <p className="text-sm font-medium text-ink">
            {value && label ? (
              <TabLink slug={value} name={label} />
            ) : loading ? (
              "Loading tab…"
            ) : (
              "Tab unavailable"
          ...
          {!label && (
            <p className="w-full text-xs text-ink-soft">
              {loading
                ? "Loading the destination tab."
                : "This tab is unavailable. Return to your tabs to start a new expense."}
            </p>
          )}
        </>
      ) : (
        <>
            ...
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger
                render={
                  <Button
                    variant="field"
                    aria-labelledby="expense-tab-label expense-tab-value"
                    aria-required="true"
                    className="group h-auto min-h-11 min-w-0 flex-1 justify-between gap-2 rounded-md px-3 py-2"
                  />
                }
              >
                <span id="expense-tab-value" className="truncate">
                  {label ??
              ...
              <PopoverContent
                align="start"
                className="w-80 max-w-[calc(100vw-3rem)] rounded-lg p-2"
              >
                <ul className="max-h-64 space-y-0.5 overflow-y-auto">
                  <li className="mb-1 border-b border-rule pb-1">
                    <MenuOption
                      ...
                      className="justify-start font-medium text-forest"
                    >
                  ...
                  {!tabs.length && (
                    <li className="px-2 py-2 text-sm text-ink-soft">
          ...
          <p className="w-full text-xs text-ink-soft">
            {label
              ? "People come from this tab. Changing tabs updates the people and their splits."
              : "Choose a tab or create one to add people and save this expense."}
          </p>
        </>
      )}
    </section>
