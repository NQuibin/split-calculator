...
test("fixed globals apply once and exclude an explicit zero override", () => {
  const result = computeSplit(people, [item("a", 100), item("b", 50), item("exempt", 80, true)], {
    ...
    tax: { mode: "amount", value: 12 },
    tip: { mode: "amount", value: 18 },
  });
...
test("individual adjustments replace globals, including zero tax and tip", () => {
  ...
  expect(computeSplit(people, [item("a", 100), own], global).items.map((i) => i.total)).toEqual([
    117, 45,
  ]);
...
test("old individual adjustments survive without a flag", () => {
  const old = {
    ...
    tax: { mode: "percent" as const, value: 5 },
  };
...
test("empty, all exempt, and fully discounted expenses receive no global charges", () => {
  ...
  expect(
    computeSplit(people, [item("a", 100)], { ...global, discount: { mode: "amount", value: 200 } })
      .grandTotal,
  ).toBe(0);
});
...
test("a fixed-amount tip ignores the after-tax flag", () => {
  const fixed: ExpenseAdjustments = {
    ...global,
    tip: { mode: "amount", value: 20 },
    tipAfterTax: true,
  };
