...
const item = (id: string, cost: number, splitWith: string[]) => ({ id, name: id, cost, splitWith, discount: zero, tax: zero, tip: zero });
...
test("a $120 receipt split 40/40/40 leaves two $40 payments to its payer", () => {
  ...
  expect(suggestSettlements(balances)).toEqual([{ fromMemberId: "b", toMemberId: "a", amount: 40 }, { fromMemberId: "c", toMemberId: "a", amount: 40 }]);
});

test("itemized 45/45/30 assigns the payer independently of the split", () => {
  const people = ["a", "b", "c"].map(id => ({ id, name: id }));
  const split = computeSplit(people, [item("a", 45, ["a"]), item("b", 45, ["b"]), item("c", 30, ["c"])]);
  const balances = computeExpenseBalances(people, split, "c");
  expect(balances.map(row => row.balance)).toEqual([-45, -45, 90]);
  expect(suggestSettlements(balances)).toEqual([{ fromMemberId: "a", toMemberId: "c", amount: 45 }, { fromMemberId: "b", toMemberId: "c", amount: 45 }]);
});
