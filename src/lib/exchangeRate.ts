...
export function activeExchangeRate(
  expense: { currency?: string; exchangeRate?: ExchangeRate },
  target: string,
): ExchangeRate | undefined {
  const rate = expense.exchangeRate;
  return rate &&
    rate.from === (expense.currency ?? "USD") &&
    rate.to === target &&
    rate.from !== target &&
...
export function convertShares(rows: ShareRow[], total: number, rate: number): ShareRow[] {
  ...
  const difference = Math.round(
    (round2(total * rate) - round2(shares.reduce((sum, share) => sum + share, 0))) * 100,
  );
  const order = rows
    .map((row, index) => ({ index, amount: row.fairShare }))
    .sort((a, b) => b.amount - a.amount);
  if (order.length)
    for (let i = 0; i < Math.abs(difference); i++)
      shares[order[i % order.length].index] = round2(
        shares[order[i % order.length].index] + Math.sign(difference) / 100,
      );
